# another way to create a shortcut, need a Etherscan_api_key for verifying
# this step cost 0.011 eth !!! succeed! But my gas price double the video's, >_<!!!
# deploy-sepolia:
...
# 'phony' means 'fake'. It tells `make` these are vitual tasks instead of real files.
...
# a shortcut for forge build
# The `:;` between `build` and `forge build` is used to indicate that the command will be given on the same line
build:; forge build
...
# why is '--account' here? Because an account can point to a keystorefile, but here is not. Running this command got failed
ifeq ($(findstring --network sepolia,$(ARGS)),--network sepolia)
	NETWORK_ARGS := --rpc-url $(SEPOLIA_RPC_URL) --account $(ACCOUNT) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif

# @ will curb the output of command. There is only result of command but command itself displayed.
# It prevent secret info from exposing in command
deploy-sepolia:
	@forge script script/DeployFundMe.s.sol:DeployFundMe $(NETWORK_ARGS)
